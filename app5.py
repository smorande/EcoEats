...
def speech_to_text():
    ...
    if audio_file is not None:
        ...
        # Save the uploaded file to a BytesIO object for OpenAI
        audio = AudioSegment.from_file(io.BytesIO(audio_bytes))
        # Convert the file to .wav format if needed (Whisper prefers WAV)
        ...
        with st.spinner('Transcribing...'):
            transcript = openai.Audio.transcribe("whisper-1", audio_wav)
        # Display the transcription
        st.subheader("Transcription:")
        st.write(transcript['text'])
        
        # Text-to-Speech functionality
        if st.button('Speak the transcription'):
            tts = gTTS(text=transcript['text'], lang='en')
            tts.save("output.mp3")
            st.audio("output.mp3")
...
def main():
    ...
    # Custom CSS for a more stylish sidebar
    st.markdown("""
        <style>
    ...
    if 'user_id' not in st.session_state:
        ...
        menu = ["Home", "Food Waste", "Healthy Eating", "Weekly Report", "Sustainability", "Goals", "Community", "Speech to Text"]
