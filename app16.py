...
def generate_ai_response(prompt, max_tokens=150):
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4o",  # Updated to use GPT-4 with vision capabilities
            messages=[
...
def create_waste_trend_chart():
    ...
    ax.plot(waste_df['date'], waste_df['quantity'], marker='o')
...
def create_meals_trend_chart():
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(meals_df['date'], meals_df['count'], marker='o')
