...
from reportlab.platypus import Image as RLImage  # Renamed to avoid conflicts
...
def generate_ai_response(prompt, max_tokens=150):
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4o-mini",  # Using gpt-4-vision-preview as a stand-in for "get-4o-mini"
            messages=[
...
def generate_pdf_report():
    ...
    img_buffer.seek(0)
    img = RLImage(img_buffer)  # Use RLImage instead of Image
    img.drawHeight = 200
    img.drawWidth = 400
    story.append(img)
    story.append(Spacer(1, 12))
...
def create_waste_trend_chart():
    ...
    sns.lineplot(data=waste_df, x='date', y='quantity', marker='o', ax=ax)  # Corrected 'px' to 'ax'
...
def main():
    # Custom CSS for a blue-colored dropdown
    st.markdown("""
        <style>
