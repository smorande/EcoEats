...
# Grocery list analysis function
...
def generate_pdf_report():
    ...
    styles.add(ParagraphStyle(name='Subtitle', fontName='Helvetica-Oblique', fontSize=14, spaceAfter=12, textColor=colors.gray))

    # Title and subtitle
    story.append(Paragraph("EcoEats Weekly Report", styles['Title']))
    story.append(Paragraph("Your journey towards sustainable eating", styles['Subtitle']))
    story.append(Spacer(1, 24))

    # Weekly Summary
    summary = generate_ai_response("Generate a comprehensive, encouraging summary of the user's progress in reducing food waste and eating healthy this week. Include specific metrics and highlight key achievements.")
    ...
    # Infographic: Progress Pie Chart
    story.append(Paragraph("Your Progress at a Glance", styles['Heading1']))
    d = Drawing(300, 200)
    pc = Pie()
    pc.x = 65
    pc.y = 15
    ...
    pc.height = 170
    pc.data = [60, 40]
    pc.labels = ['Progress', 'Room for Improvement']
    pc.slices.strokeWidth = 0.5
    pc.slices[0].fillColor = colors.green
    pc.slices[1].fillColor = colors.lightgrey
    d.add(pc)
    story.append(d)
    story.append(Spacer(1, 12))

    # Sustainability Tips
    story.append(Paragraph("Sustainability Tips for Next Week", styles['Heading1']))
    tips = generate_ai_response("Provide 3 actionable, engaging tips for reducing food waste and eating more sustainably. Make them specific and tailored to the user's progress.")
    for i, tip in enumerate(tips.split('\n'), 1):
        story.append(Paragraph(f"{i}. {tip}", styles['BodyText']))
    story.append(Spacer(1, 12))

    # Closing message
    story.append(Paragraph("Keep up the great work! Every small action counts towards a more sustainable future.", styles['Subtitle']))
...
def weekly_report():
    ...
    if st.button("Generate PDF Report"):
        ...
        st.download_button(
            label="Download Your Personalized Sustainability Report",
            data=pdf_buffer,
            file_name="EcoEats_Weekly_Report.pdf",
...
def ml_utilities():
    st.subheader("🤖 ML Utilities")
    
    # Visitor counter
    if 'ml_utilities_visits' not in st.session_state:
        st.session_state.ml_utilities_visits = 0
    st.session_state.ml_utilities_visits += 1
    
    st.write(f"Visitor count: {st.session_state.ml_utilities_visits}")
    
    # Create tabs for different ML features
    tabs = st.tabs(["AI Meal Planner", "Image Food Logger", "Sustainability Coach", "Recipe Generator", "Waste Predictor"])
...
def ai_meal_planner():
    ...
    dietary_preferences = st.multiselect("Select dietary preferences:", 
                                         ["Vegetarian", "Vegan", "Gluten-Free", "Low-Carb", "Keto"])
    health_goals = st.text_input("Enter your health goals:")
    
    if st.button("Generate Meal Plan"):
        prompt = f"Create a 7-day meal plan for someone with {', '.join(dietary_preferences)} dietary preferences and the following health goals: {health_goals}. Include breakfast, lunch, dinner, and snacks for each day."
        meal_plan = generate_ai_response(prompt, max_tokens=1000)
        st.write(meal_plan)

def image_food_logger():
    st.subheader("Image-Based Food Logger")
    uploaded_image = st.file_uploader("Upload a photo of your meal", type=["jpg", "jpeg", "png"])
    if uploaded_image:
        image = Image.open(uploaded_image)
        st.image(image, caption="Uploaded Meal", use_column_width=True)
        
        analysis = analyze_image(uploaded_image, "Identify the food items in this image, estimate their quantities, and provide nutritional information.")
        st.write("AI Analysis:", analysis)
        
        if st.button("Log This Meal"):
            # Logic to save the meal data
            c.execute("INSERT INTO meals (meal, nutrition, date, image) VALUES (?, ?, ?, ?)", 
                      ("AI Logged Meal", analysis, datetime.now().isoformat(), uploaded_image.getvalue()))
            conn.commit()
            st.success("Meal logged successfully!")

def sustainability_coach():
    st.subheader("Your AI Sustainability Coach")
    user_habits = st.text_area("Describe your current eating and shopping habits:")
    if st.button("Get Personalized Advice"):
        prompt = f"Based on these habits: {user_habits}, provide personalized advice to improve sustainability in food consumption and reduce waste. Include 3-5 actionable tips."
...
def recipe_generator():
    ...
    cuisine_type = st.selectbox("Select cuisine type:", ["Any", "Italian", "Chinese", "Mexican", "Indian", "French"])
    
    if st.button("Generate Recipe"):
        prompt = f"Create a recipe using these ingredients: {ingredients}. Cuisine type: {cuisine_type}. Provide a list of ingredients with quantities and step-by-step instructions."
        recipe = generate_ai_response(prompt, max_tokens=500)
        st.write(recipe)

def food_waste_predictor():
    st.subheader("AI Food Waste Predictor")
    
    c.execute("SELECT item, quantity, date FROM food_waste ORDER BY date DESC LIMIT 50")
    waste_data = c.fetchall()
    
    if waste_data:
        df = pd.DataFrame(waste_data, columns=['item', 'quantity', 'date'])
        df['date'] = pd.to_datetime(df['date'])
        
        prediction_prompt = f"Based on this food waste data: {df.to_string()}, predict potential food waste for the next week. Suggest items that might be at risk of being wasted and provide tips to prevent waste."
        prediction = generate_ai_response(prediction_prompt, max_tokens=300)
        
        st.write("Food Waste Prediction and Tips:")
        st.info(prediction)
    else:
        st.write("Not enough data to make a prediction. Keep logging your food waste!")
...
def main():
    ...
    menu = ["Home", "Food Waste", "Healthy Eating", "Weekly Report", "Sustainability", 
            "Goals", "Community", "ML Utilities"]
